...
export class HttpRequest {
  ...
  constructor(
    private readonly options: {
      ...
      readonly clientDevice: string | null;
    ...
    this.axiosInstance = axios.create({
      headers: {
        ...
        ...(options.clientDevice ? { 'X-Client-Device': options.clientDevice } : {}),
      },
  ...
  private readonly onRejected = async (error: unknown): Promise<never> => {
    ...
    if (error instanceof Error) {
      throw new Exception(error.message || 'Unhandled HTTP client error.', { cause: error });
    }

    throw new Exception('Unknown HTTP client error.', { cause: error });
  };
  ...
  private request<T = unknown, R = T, D = unknown>(
    method: 'get' | 'post' | 'patch' | 'put' | 'delete',
    url: string,
    data?: D,
    config?: AxiosRequestConfig<D>,
